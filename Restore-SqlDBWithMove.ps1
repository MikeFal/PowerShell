...
$relocate = @()
$dbname = (Invoke-Sqlcmd -ServerInstance localhost -Database tempdb -Query "RESTORE HEADERONLY FROM DISK='$BackupFile';").DatabaseName
...
If($OutputPath -ne 'NoPath'){
    ...
    Restore-SqlDatabase -ServerInstance localhost -Database $dbname -RelocateFile $relocate -BackupFile "$BackupFile" -RestoreAction Database -Script | Out-File $OutputFile
} else {
    Restore-SqlDatabase -ServerInstance localhost -Database $dbname -RelocateFile $relocate -BackupFile "$BackupFile" -RestoreAction Database 
}
}